...
def plotPDF(fileHandle, data, hitPoints, ccpwrsPoints, ccpwrs, coarseID):
	...
	def powerHist():
		...
		# Get DETPOW/MEANPOW
		relpow = [row[0]/row[1] for table in data for row in table if row[1] != 0]
...
def metaSummary(fileHandle, data, nSteps, hitPoints, outDir):
	"""
	...
	def jsonMeta(meta, outDir):
		...
		def jsonAdd(data, meta):
			"""
			Adds new data to stored json data
			"""

			# Year-week time stamp
			digits = [i for i in filename.split('_') if i.isdigit()]
			digits = [i for i in digits if len(i) == 8]
			if len(digits) > 1:
				for i in digits:
					if i[1:2] == '20':
			...
			data[timeKey] = data.get(timeKey) or {}

			data[timeKey]['DURATION (MINS)'] = data.get(timeKey).get('DURATION (MINS)', 0) + \
				float(meta['DURATION'].split()[0])

			return data

		# Summary file location
		jsonPath = outDir + 'summary.txt'
		jsonPath = os.path.expanduser(jsonPath)

		# Append new data
		data = jsonImport(jsonPath)
		data = jsonAdd(data, meta)

		# Save data to summary file
		jsonExport(jsonPath, data)

		return True

	meta = getMeta()

	jsonMeta(meta, outDir)
	drawPage(meta) # Creates summary page
...
def main(filename):
	...
	metaSummary(fileHandle, data, nSteps, hitPoints, outDir)
...
if __name__ == "__main__":
	...
	filepath= fsplit[0] + '/'
	filename = os.path.splitext(fsplit[1])[0]
	outDir = sys.argv[2]
